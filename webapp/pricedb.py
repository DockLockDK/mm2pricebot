...
каждый цикл проверки — рабочая схема, но с двумя проблемами на дистанции:
  ...
  2. Чтение при этом ограничено последними *_LOG_READ_LIMIT строками —
     то есть после ~14 дней (20000 строк / 1440 циклов в день при
     CHECK_INTERVAL_SEC=60) старая история физически ещё на диске, но
...
проверки (обычно раз в минуту), после HOT_RETENTION_DAYS — почасовыми
