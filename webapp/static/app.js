...
const I18N = {
  ...
  faq8_a: { ru: "Для коротких периодов «было → стало» (до суток) график показывает всю накопленную историю по минутным точкам, а не только выбранный период — иначе он был бы почти пустым. Для недели, месяца и года график показывает ровно этот период, точки крупнее: 4-часовые, дневные, недельные — чтобы линия оставалась читаемой.", en: "For short \"then → now\" periods (up to a day) the chart shows all accumulated history in 1-minute points, not just the selected period — otherwise it would be almost empty. For a week, month, or year, the chart shows exactly that period, with coarser points: 4-hour, daily, weekly — so the line stays readable." },
  ...
  bucket_hourly_day: { ru: "минутные точки · вся история", en: "1-minute points · full history" },
...
function showScreen(name) {
  ...
  el("#app-header").classList.toggle("home-header", name === "home");
  if (tg) {
