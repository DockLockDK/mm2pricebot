...
# включительно) график показывает ВСЮ накопленную историю (None — без
# ограничения снизу, см. build_candles/build_rarity_index), а не обрезается
# по времени; длинные периоды (неделя и больше) показывают ровно себя, там
# смысл "растянуть план" уже есть. bucket_sec — во сколько укрупняются
# точки/свечи; чем длиннее реальная история, тем крупнее становятся бакеты
# сами по себе благодаря retention-уплотнению в pricedb.compact(), так что
# число точек на графике не растёт неограниченно.
...
def resolve_chart_window(window):
    return CHART_WINDOW_SECONDS.get(window)
